...
int main() {
    ...
    printf("Digite o nome do manga que deseja ler: ");

    // fgets lê o que o usuário digita (seguro contra overflow de memória)
    if (fgets(nome_digitado, sizeof(nome_digitado), stdin) == NULL) {
        printf("Erro ao ler a entrada.\n");
        return 1;
    }

    // 2. Formata para o padrão da internet (URL Encoding)
    char nome_formatado[300];
    formatar_texto_url(nome_formatado, nome_digitado);

    // 3. Monta a URL dinâmica
    char url_manga[512];
